...
void expand(char *string, char *expanded_string) {
    char lower_value, upper_value = 0; // Переменные "от", "до"
    ...
    for (int j = 0; string[j] != '\0'; ++j) {
        ...
        if (string[0] == '-' && j == 0) {
            for (j = 0; string[j] == '-'; ++j) {
                input_value_into_expanded_string(k, string[j], expanded_string);
                ++k;
            }
        }
        ...
        if (count == 2) {
            ...
            if (lower_value < upper_value) {
                for (char value = lower_value; value <= upper_value; ++value) {
                    input_value_into_expanded_string(k, value, expanded_string);
                    ++k;
                }
                count = 0;
            }
            ...
            if (string[j + 1] == '-' && (string[j + 2] == '-' || string[j + 2] == '\0')) {
                for (int i = j + 1; string[i] != '\0'; ++i) {
                    input_value_into_expanded_string(k, '-', expanded_string);
                    ++k;
                }
                break;
            }
